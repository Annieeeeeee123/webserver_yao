...
vector<vector<string>> mysql_con::  getDatafromDB(string table_name, string user_name) 
{
...
bool mysql_con::InsertDB(string table_name, string m_name, string pass) 
{
	if (m_name.length() == 0) {
		cout<<"人名不能为空!"<<endl;
		return false;
...
bool mysql_con::DeleteDB(string table_name, string m_name) {
	if (m_name.length() == 0) {
		cout<<"人名不能为空!"<<endl;
		return false;
	}
	...
	if (mysql_query(mysql, query.c_str())) {
		cout<<"mysql delete error"<<endl;
		return false;
	}
	return true;
}

bool mysql_con::UpdateDB(string table_name, int user_id, string m_name, string pass) {
	if (m_name.length() == 0) {
		cout<<"人名不能为空!"<<endl;
		return false;
	}
